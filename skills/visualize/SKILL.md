...
> **Дефолтный формат финала — .jpeg.** После генерации HTML обязательно конвертировать в JPEG командой:
> `.claude/scripts/html-to-jpg.sh <input.html> [output.jpg] [width] [height]`
> По умолчанию width=1400, height=3500 (полный скриншот). Скрипт делает headless Chrome screenshot → sips JPEG качество 85. Отдавать owner'у файл .jpg, исходный .html сохранять рядом для правок. Исключение — owner явно сказал «оставь как HTML».
