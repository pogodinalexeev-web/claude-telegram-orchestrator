...
> **Дефолтный формат финала — .jpeg.** Если owner не сказал «оставь как .excalidraw / .svg» явно — после генерации .excalidraw JSON отрендерить через `@excalidraw/excalidraw` headless или `excalidraw_export` CLI в PNG → конвертировать в .jpg (качество 85). Отдавать owner'у файл .jpg, .excalidraw сохранять рядом для правок.
